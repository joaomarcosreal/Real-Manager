...
uses
  ...
  RXDBCtrl, DBCtrls, Buttons, Mask, ExtCtrls, ibx.ibquery,db, ComCtrls;

type
  TfrmEmissaoNFE = class(Tfrm_modelo_cadastro)
    Label7: TLabel;
    Panel8: TPanel;
    Label46: TLabel;
    pnFornecedor: TPanel;
...
procedure TfrmEmissaoNFE.novaNota();
begin
...
procedure TfrmEmissaoNFE.bt_novoClick(Sender: TObject);
begin
  inherited;
  novaNota();
...
procedure TfrmEmissaoNFE.retornaItensSaida(cod_saida : string);
var
 q : TIBQuery;
begin
...
procedure TfrmEmissaoNFE.adicionaItemSaida(ordem  : string;
                                           codigo : string;
                                           descricao : string;
                                           quantidade : double;
                                           codUnidade : string;
...
procedure TfrmEmissaoNFE.limpaEditsItem();
begin
  ed_desc_item.clear;
...
procedure TfrmEmissaoNFE.recalculaTotaisNota();
var
...
function TfrmEmissaoNFE.ValidaTotaisNota() : boolean;
var
totalBaseICMS, totalICMS,totalBaseICMSSUB,totalICMSSUB,totalProdutos,totalIPI,totalDescontos, totalOutrasDespesas : currency;
...
procedure TfrmEmissaoNFE.retornaInformacoes(cod_saida : string);
begin
...
procedure TfrmEmissaoNFE.edNumeroNotaFiscalExit(Sender: TObject);
var
 codSaida : string;
 modelo : string;
begin
...
function TfrmEmissaoNFE.verificaValidadeLancamentoItemSaida() : boolean;
var
 cfop : string;
 tipoCfop : integer;
...
procedure TfrmEmissaoNFE.bt_consultarClick(Sender: TObject);
begin
...
procedure TfrmEmissaoNFE.Edit1Exit(Sender: TObject);
begin
  inherited;
...
procedure TfrmEmissaoNFE.Edit1Enter(Sender: TObject);
begin
  inherited;
...
procedure TfrmEmissaoNFE.DBGrid1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  inherited;
...
procedure TfrmEmissaoNFE.DBGrid1ColExit(Sender: TObject);
begin
  inherited;
    recalculaTotaisNota();
end;
...
function TfrmEmissaoNFE.verificaValidade() : boolean;
var
...
procedure TfrmEmissaoNFE.gravarInformacoes();
begin
 if (evento = 1) then
   begin
...
procedure TfrmEmissaoNFE.gravarItensSaidas();
var
 q : TIBQuery;
 ordem : integer;
begin
...
procedure TfrmEmissaoNFE.bt_gravarClick(Sender: TObject);
begin
...
procedure TfrmEmissaoNFE.ed_codigoExit(Sender: TObject);
begin
  inherited;
...
procedure TfrmEmissaoNFE.bt_cancelarClick(Sender: TObject);
begin
  inherited;
...
procedure TfrmEmissaoNFE.imprimirEspelho(codigo_saida : string);
begin
...
procedure TfrmEmissaoNFE.bt_imprimirClick(Sender: TObject);
begin
  inherited;
  imprimirEspelho(ed_codigo.Text);
end;

procedure TfrmEmissaoNFE.bt_pesquisaClick(Sender: TObject);
begin
  inherited;
  pesquisaSaidas();
...
procedure TfrmEmissaoNFE.copiaValoresNota();
begin
...
procedure TfrmEmissaoNFE.retornaValoresNota();
begin
...
procedure TfrmEmissaoNFE.Edit1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  inherited;
...
procedure TfrmEmissaoNFE.ckbCupomAnexoClick(Sender: TObject);
var
 str : string;
begin
  inherited;
       ...
       if ( (dm.qry_saidas.State = dsEdit)  or (dm.qry_saidas.State = dsInsert) ) then
        begin
         ...
         if (dm.qry_saidas.State = dsInsert)  then
          begin
            dm.qry_saidas.fieldbyname('CFOP_NOTA').value := '5929';

            exibeInformacoesCFOPEntrada('5929');
          end;
        end;
       pnNumeroCupom.Visible := true;
...
procedure TfrmEmissaoNFE.rdVendaClick(Sender: TObject);
begin
  inherited;
...
procedure TfrmEmissaoNFE.rdDevolucaoClick(Sender: TObject);
begin
  inherited;
...
procedure TfrmEmissaoNFE.rdTransferenciaClick(Sender: TObject);
begin
  inherited;
...
procedure TfrmEmissaoNFE.rdModelo01Click(Sender: TObject);
begin
  inherited;
...
procedure TfrmEmissaoNFE.rdNotaEletronicaClick(Sender: TObject);
begin
  inherited;
  if ( (dm.qry_saidas.State = dsEdit)  or (dm.qry_saidas.State = dsInsert) ) then
    begin

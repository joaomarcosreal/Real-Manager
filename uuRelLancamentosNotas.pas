...
uses
  ...
  ACBrUtil, Dialogs, uu_modelo_Vazio, StdCtrls, ExtCtrls, Buttons,
  RxToolEdit, JvActionsEngine, JvControlActions, Vcl.ActnList, System.Actions,
  uuDMDRE, PngSpeedButton, Vcl.Mask, Vcl.Menus;
...
uses
  uuDmRelatorios, uu_frm_principal;
...
begin
  ...
  dRelatorios.qryNotasRDPeriodo.sql.Add('        where cod_emp_grupo in (select se.cod_empresa from selecao_empresas se) ');
  ...
  dRelatorios.setFiltroRelatorio(trim(dmDRE.qryDRE.fieldbyname('grupo').Value));
  dRelatorios.rpt.ShowReport(true);
  dRelatorios.qryNotasRDPeriodo.Active := false;

end;

procedure TfrmRelLancamentosNotasRD.PngSpeedButton1Click(Sender: TObject);
begin
  inherited;
  close;
end;

procedure TfrmRelLancamentosNotasRD.PngSpeedButton2Click(Sender: TObject);
begin
  inherited;
...
procedure TfrmRelLancamentosNotasRD.PngSpeedButton3Click(Sender: TObject);
begin
  inherited;
  close;
end;

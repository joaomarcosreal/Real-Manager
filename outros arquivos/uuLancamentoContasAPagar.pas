...
uses
  ...
  Dialogs, uu_modelo, StdCtrls, Buttons, Mask, ExtCtrls, DB, RxMemDS,
  ibx.ibquery, DBCtrls, Grids, DBGrids, ComCtrls, Menus, RxCurrEdit, RxToolEdit;
...
uses
  uu_data_module, uu_frm_principal, uu_modelo_vazio, uuBaixaTitulosContasApagar;
...
begin
  ...
  case evento of
    1:
      mensagem := 'Confirma lançamento ?';
    2:
      mensagem := 'Confirma alteração ?';
    3:
...
begin
 ...
 ////
  ...
  case evento of
    ...
    3:
      showmessage('Exclusão efetuada com suceso!');
  end;
...
//  bt_novo.Click;
  ...
  case evento of
    2:
      begin
        lbTitulo.Caption := 'Contas a pagar - Alteração de título';
        bt_alterar.Click;
//           edCodLoja.SetFocus;
    ...
    3:
      begin
        lbTitulo.Caption := 'Contas a pagar - Exclusão de título';
        retornaInformacoes(ed_codigo.Text);
...
procedure TfrmLancamentoContasApagar.DBGrid1DrawColumnCell(Sender: TObject; const Rect: TRect; DataCol: Integer; Column: TColumn; State: TGridDrawState);
begin
  inherited;
...
procedure TfrmLancamentoContasApagar.FormKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
//  inherited;
...
procedure TfrmLancamentoContasApagar.ExcluirTitulo;
var
  msg: string;
  codContaAPagar: string;
...
begin
  ...
  dm.qryauxiliar.Active := false;
  dm.qryauxiliar.SQL.clear;
  dm.qryauxiliar.sql.add('delete from lancamentos_rd lr where  lr.cod_ct_pagar=' + Quotedstr(codContaSelecionada));
  dm.qryauxiliar.ExecSQL;

  dm.qryauxiliar.Active := false;
  dm.qryauxiliar.SQL.clear;
  dm.qryauxiliar.sql.add('delete from  contas_a_pagar cp  where cp.codigo =' + Quotedstr(codContaSelecionada));
  dm.qryauxiliar.ExecSQL;

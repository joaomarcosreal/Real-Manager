...
uses
  ...
  Dialogs, uu_modelo_vazio, StdCtrls, ExtCtrls, Buttons, RxLookup,RxCurrEdit, Mask,
    AdvTouchKeyboard, RxToolEdit, Vcl.ActnList, System.Actions;
...
procedure Tfrm_cancela_item_restaurante.btConfirmarClick(Sender: TObject);
var
obs : string;
ct : integer;
begin
...
begin
  ...
  if edQuantidade.Value <= 0 then
   ...
   if( edQuantidade.value > dm.qry_movimento_mesa.FieldByName('quantidade').Value ) then
    begin

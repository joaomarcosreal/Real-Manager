...
uses
  ...
  Dialogs, uu_modelo_Vazio, Mask, StdCtrls, ExtCtrls, Buttons, OnGuard,ogutil,ibquery;
...
uses uu_frm_principal, uu_data_module, uuParabensRegistro;
  ...
  if ValidaSerial = true then
   begin
     ...
     if HexToBuffer(serial, Work, SizeOf(Work)) then
      begin
        dm.gravaini('BANCO_DE_DADOS','release',dm.criptografa(serial,'1320'));

...
procedure Tfrm_transferir_item.AdvGlassButton1Click(Sender: TObject);
var
obs : string;
begin
  inherited;
...
//  dm.gravarTransferenciaDeItem(frm_caixa_restaurante.edNumeroMesa.Text,ed_mesa_destino.Text,cmbMotivo.KeyValue,cmbMotivo.Text,
//  dm.qryMovimentoMesa.fieldbyname('cod_barras').value,dm.qryMovimentoMesa.fieldbyname('descricao').value,StrToFloat(lbQtde.Caption)  ,(dm.qryMovimentoMesa.fieldbyname('valor_unitario').value),obs);
  ...
  dm.qryauxiliar.Active:=False;
  dm.qryauxiliar.sql.clear;
  dm.qryauxiliar.sql.add('update movimento_mesa set cod_mesa='+Quotedstr(ed_mesa_destino.Text)+',obs_paf='+Quotedstr('Transf. mesa '+IntToStr(StrToInt( dm.qryMovimentoMesa.fieldbyname('cod_mesa').value))));
  dm.qryauxiliar.sql.add(',cod_movimento='+Quotedstr(codigo_lancamento)+' where cod_mesa='+Quotedstr(dm.qryMovimentoMesa.fieldbyname('cod_mesa').value));
  dm.qryauxiliar.sql.add(' and cod_movimento='+Quotedstr(dm.qryMovimentoMesa.fieldbyname('cod_movimento').value));
...
procedure Tfrm_transferir_item.AdvGlassButton3Click(Sender: TObject);
begin
  inherited;
  close;
end;

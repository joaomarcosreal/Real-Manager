...
uses
  ...
  ExtCtrls, Buttons, DBCtrls, Menus, Data.DB, RxToolEdit, JvExMask, JvSpin,
...
uses UU_DM_RELATORIOS, uu_data_module, uu_frm_principal, DateUtils;

{$R *.dfm}

procedure TfrmRelProdutosVariosPeriodos.BitBtn2Click(Sender: TObject);
begin
  inherited;
  close;;
end;
...
procedure TfrmRelProdutosVariosPeriodos.BitBtn1Click(Sender: TObject);
var
 aliasPeriodo : string;
begin
  inherited;
  ...
  while not dm_relatorios.cdsRelProdutosVendidosTemp.Eof do
   begin
     ...
     dm_relatorios.qryRelComparativoProdutos.sql.add(' pm.descricao, pm.quantidade_contagem, pm.total, pm.percentualproduto,');

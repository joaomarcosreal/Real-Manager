...
type
  Tfrm_cad_usuarios = class(TForm)
    ...
    procedure gravar_permissoes ;
    procedure bt_novoClick(Sender: TObject);
    procedure ed_cod_usuarioKeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
    ...
    procedure revisao_permissoes(cod_usuario : string);
  private
...
uses uu_frm_principal, uu_data_module, uu_cs_usuarios;

{$R *.dfm}

function Tfrm_cad_usuarios.controla_botoes(botao : TBitBtn) : boolean;
begin
...
procedure Tfrm_cad_usuarios.ed_cod_usuarioKeyDown(Sender: TObject;
...
begin
  ...
  1 : begin
        ...
        dm.qryauxiliar.SQL.add('insert into usuarios (cod_usuario,nome_usuario,funcao,login,senha,entradas,acesso) values (');
        dm.qryauxiliar.sql.add(''+QuotedStr(ed_cod_usuario.Text)+',');
        dm.qryauxiliar.sql.add(''+QuotedStr(ed_nome.text)+',');
        dm.qryauxiliar.sql.add(''+Quotedstr(ed_cargo.text)+',');
        dm.qryauxiliar.sql.add(''+Quotedstr(ed_login.text)+',');
        dm.qryauxiliar.sql.add(''+Quotedstr(dm.tb_parametros.fieldbyname('senha_padrao').value)+',0,'+Quotedstr('01060101')+')');
      end;

  2 : begin
        dm.qryauxiliar.active:=false;
        dm.qryauxiliar.sql.clear;
        dm.qryauxiliar.SQL.add('update usuarios set ');
        dm.qryauxiliar.sql.add('nome_usuario='+QuotedStr(ed_nome.text)+',');
        dm.qryauxiliar.sql.add('funcao='+Quotedstr(ed_cargo.text)+',');
        dm.qryauxiliar.sql.add('login='+Quotedstr(ed_login.text)+' ');
        if (ckbRedefinir.Checked  = true) then
         begin
           dm.qryauxiliar.sql.add(',senha='+Quotedstr(dm.tb_parametros.fieldbyname('senha_padrao').value)+' ');
         end;



        dm.qryauxiliar.sql.add('where cod_usuario='+Quotedstr(ed_cod_usuario.text));
      end;
  3 : begin
        dm.qryauxiliar.Active:=false;
        dm.qryauxiliar.sql.clear;
        dm.qryauxiliar.sql.add('delete from permissoes where cod_usuario='+Quotedstr(ed_cod_usuario.text));
        ...
        try
          ...
          begin
            dm.cria_log_de_erros(dm.qryauxiliar.sql,'Cadastro de usuários','Erro ao excluir permissões de usuário ',codigo_usuario);
          end;
        end;
        dm.qryauxiliar.Active:=false;
        dm.qryauxiliar.sql.clear;
        dm.qryauxiliar.sql.add('delete from usuarios where cod_usuario='+Quotedstr(ed_cod_usuario.text));
  ...
  try
   ...
   begin
     case oquefazer of
      1: dm.cria_log_de_erros(dm.qryauxiliar.sql,'Cadastro de usuários','Erro incluir usuário ',codigo_usuario);
      2: dm.cria_log_de_erros(dm.qryauxiliar.sql,'Cadastro de usuários','Erro ao alterar usuário ',codigo_usuario);
      3: dm.cria_log_de_erros(dm.qryauxiliar.sql,'Cadastro de usuários','Erro ao excluir usuário ',codigo_usuario);
     end;
   end;
   Result:=false;
   exit;
...
begin
  ...
  while item_atual <> total_itens do
  begin
   If ckb_submodulos.Checked[item_atual] then total_selecionados:=total_selecionados+1;
   item_atual:=item_atual+1;
  end;
...
begin
 ...
 if  sender is Tedit and ((sender as Tedit).Name='ed_cod_usuario') then
 begin
   if trim(ed_cod_usuario.Text)='' then exit;
   Try
    begin
      ed_cod_usuario.text:=FormatFloat('000',Strtofloat(ed_cod_usuario.text));
    end;
...
procedure Tfrm_cad_usuarios.gravar_permissoes ;
begin
  ...
  try
   ...
   begin
     dm.cria_log_de_erros(dm.qryauxiliar.sql,'Cadastro de usuários','Erro ao excluir permissões',codigo_usuario);
     exit;
   end;
  end;
  ...
  begin
    ...
    dm.qryauxiliar.Active:=false;
    dm.qryauxiliar.SQL.Clear;
    dm.qryauxiliar.SQL.add('insert into permissoes (cod_usuario,cod_modulo,cod_mod,permite) values (');
    dm.qryauxiliar.sql.add(''+QuotedStr(ed_cod_usuario.Text)+',');
    dm.qryauxiliar.sql.add(''+Quotedstr(dm.modulos_tempcod_modulo.Value)+',');
    dm.qryauxiliar.sql.add(''+QuotedStr(dm.modulos_tempcod_mod.Value)+',');
    dm.qryauxiliar.sql.add(''+IntToStr(dm.modulos_temppermite.Value)+')');
    ...
    try
      ...
      begin
        dm.cria_log_de_erros(dm.qryauxiliar.sql,'Cadastro de usuários','Erro ao incluir permissão de módulo',codigo_usuario);
        exit;
      end;
    end;
  ...
  begin
    ...
    dm.qryauxiliar.SQL.add('insert into permissoes (cod_usuario,cod_modulo,cod_mod,permite) values (');
    dm.qryauxiliar.sql.add(''+QuotedStr(ed_cod_usuario.Text)+',');
    dm.qryauxiliar.sql.add(''+Quotedstr(dm.submodulos_tempcod_modulo.Value)+',');
    dm.qryauxiliar.sql.add(''+QuotedStr(dm.submodulos_tempcod_mod.Value)+',');
    dm.qryauxiliar.sql.add(''+IntToStr(dm.submodulos_temppermite.Value)+')');
    try
      begin
        dm.qryauxiliar.ExecSQL;
      end;
      ...
      begin
        dm.cria_log_de_erros(dm.qryauxiliar.sql,'Cadastro de usuários','Erro ao incluir permissão de submódulo',codigo_usuario);
        exit;
      end;
    end;
    dm.submodulos_temp.Next;
...
procedure Tfrm_cad_usuarios.bt_novoKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if key=vk_escape then bt_sair.Click;
end;
...
begin
  ...
  revisao_permissoes(ed_cod_usuario.Text);
  ...
  if oquefazer<> 3 then ed_nome.SetFocus;
...
procedure Tfrm_cad_usuarios.revisao_permissoes(cod_usuario : string);
begin
 dm.qryauxiliar.active:=false;
 dm.qryauxiliar.sql.Clear;
 dm.qryauxiliar.SQL.add('select * from modulos order by cod_modulo,cod_mod');
 ...
 begin
   ...
   dm.qryauxiliar2.SQL.add('select * from permissoes where cod_modulo='+Quotedstr(dm.qryauxiliar.fieldbyname('cod_modulo').value));
   dm.qryauxiliar2.sql.Add(' and cod_mod='+Quotedstr(dm.qryauxiliar.fieldbyname('cod_mod').value));
   dm.qryauxiliar2.sql.Add(' and cod_usuario='+Quotedstr(cod_usuario));
   ...
   begin
     dm.qryauxiliar2.Active:=false;
     dm.qryauxiliar2.sql.clear;
     dm.qryauxiliar2.sql.add('insert into permissoes (cod_modulo,cod_mod,cod_usuario,permite) values (');
     dm.qryauxiliar2.sql.add(''+Quotedstr(dm.qryauxiliar.fieldbyname('cod_modulo').value));
     dm.qryauxiliar2.sql.add(','+Quotedstr(dm.qryauxiliar.fieldbyname('cod_mod').value));
     dm.qryauxiliar2.sql.add(','+Quotedstr(cod_usuario)+',0)');

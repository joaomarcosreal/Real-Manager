...
uses
  ...
  cxDBLookupEdit, cxDBLookupComboBox, JvDBLookup;
...
type
  TfrmPedidosDelivery = class(Tfrm_modelo_vazio)
    ...
    edReferenciaCliente: TEdit;
    Panel19: TPanel;
    Shape2: TShape;
    Label45: TLabel;
    pnTitulo: TPanel;
    lbTitulo: TLabel;
    pnMovimentoPedido: TPanel;
    Panel7: TPanel;
    Label17: TLabel;
...
begin
  ...
  try
    try
      ...
      edReferenciaCliente.text := q.fieldbyname('referencia1').value + ',' + q.fieldbyname('referencia2').value;
      edTelClienteConsulta.text := q.fieldbyname('telefone').value;
      ...
      begin
        ...
        begin
          msgPedidoExistente := sLineBreak + 'Já existe um pedido desse cliente lançado na data corrente!!';
          ...
          if (ShowRealDialog(Self, tmConfirmacao, 'Confirme', msgPedidoExistente, 16) = teSim) then
          begin
  ...
  try
    try
      ...
      begin
        ...
        LSQL.add('numero_caixa,cod_usuario,valor_unitario,id,observacoes,sabores,tipo_massa, tabela_preco) values (');
...
begin
  ...
  dm.qryauxiliar.SQL.add('numero_caixa,cod_usuario,valor_unitario,id,observacoes,sabores,tipo_massa, tabela_preco) values (');
...
begin
  ...
  try
    ...
    {
      LSQL.Add('select mv.cod_mesa,pr.cod_barras,pr.un_ecf,');
      ...
      LSQL.Add('udf_truncdec((mv.valor_unitario * mv.quantidade),3) as total')
      else
      LSQL.Add('udf_rounddec((mv.valor_unitario * mv.quantidade),3) as total ');

      LSQL.Add(',mv.comanda, mv.cod_garcon, g.apelido,u.login,mv.hora_lancamento, mv.estado,mv.cod_movimento,id, impresso,observacoes,sabores,tipo_massa,');

      LSQL.Add('  coalesce((select coalesce(sum(sb.desconto),0) ');
      LSQL.Add('   from sabores_pizza_movimento_mesa sb where sb.id_mov_mesa = mv.id) + mv.desconto,0) as desconto ');
      ...
      LSQL.Add('  where mv.cod_mesa=' + Quotedstr(mesaDelivery) + ' order by mv.id asc ');
    }
...
begin
  ...
  try
    try
      ...
      q.SQL.add('            select ' + Quotedstr(AcodigoPedido) + ',mv.cod_produto,mv.cod_garcon,mv.cod_movimento,mv.numero_caixa,');
...
begin
  ...
  begin
    ...
    begin
      if (pgListMovimentoProdutos.ActivePageIndex = 2) then
      begin
        pgProdutosPedido.Show;
      end
      else
...
begin
  ...
  if dmPedidoDelivery.tbFormasPagamentoTemp.IsEmpty = false then
    if (ShowRealDialog(Self, tmConfirmacao, 'Confirme', 'Excluir forma de pagamento?', 16) = teSim) then
    begin

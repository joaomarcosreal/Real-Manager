...
begin
  if TipoRelatorio = 'V' then
     frmTesteECF.ACBrECF1.CupomVinculado( edCupom.Text, edFPG.Text, edCNF.Text,
        StrToFloatDef( edValor.Text,0 ), mRelat.Lines,
        StrToIntDef(edVias.Text,1) )
  else
//     frmTesteECF.ACBrECF1.RelatorioGerencial(mRelat.Lines,StrToIntDef(edVias.Text,1),0);
end;

procedure TfrRelatorio.FormShow(Sender: TObject);
begin
//  mRelat.WrapAtValue := frmTesteECF.ACBrECF1.Colunas ;
  ...
  if edCupom.Visible then
     edCupom.Text := frmTesteECF.ACBrECF1.NumCupom ;
     
  edCNF.Text      := '' ;
  edCNF.Visible   := edFPG.Visible and
                    (frmTesteECF.ACBrECF1.Modelo in [ecfDaruma, ecfSchalter]) ;
...
begin
  frmTesteECF.FormasdePagamento1Click( Sender );
end;
...
begin
  frmTesteECF.CarregaComprovantesNAOFiscais1Click( Sender );
end;

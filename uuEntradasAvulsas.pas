...
uses
  ...
  RealFramework, Dialogs, ibx.ibquery, uu_modelo, StdCtrls, DBCtrls, Buttons,
  Mask, ExtCtrls, RXDBCtrl, ComCtrls, xmldom, Xmlxform, DB, DBClient, Menus,
  RxCurrEdit, RxToolEdit, cxGraphics, cxControls, cxLookAndFeels,
  cxLookAndFeelPainters, cxStyles, cxDataStorage, cxEdit, cxNavigator, cxDBData,
  cxGridLevel, cxClasses, cxGridCustomView, cxGridCustomTableView,
  cxGridTableView, cxGridDBTableView, cxGrid, cxCurrencyEdit, cxCustomData,
  cxFilter, cxData, dxDateRanges, dxScrollbarAnnotations, Datasnap.Provider;
...
type
  ...
  private
    ...
    LCodFornecedor, LNomeFornecedor, LNumeroNotaFiscal, cnpjFornecedor, codLoja, descricaoLoja: string;
...
uses
  uu_data_module, uu_modelo_vazio, uu_frm_principal, uu_cad_fornecedor,
  uu_produtos, UU_DM_RELATORIOS, uuLancamentosRD, uuContasAPagarEntradas,
  uuDmContasAPagar;

{$R *.dfm}

procedure TfrmEntradasAvulsas.FormCreate(Sender: TObject);
begin
  inherited;
...
procedure TfrmEntradasAvulsas.adicionaItemCompra(ordem: string; codigo: string; descricao: string; quantidade: double; codUnidade: string; siglaUnidade: string; preco: double; CST: string; CFOP: string; desconto: double; outrasDespesasAcessorias: double; baseICMS: double; valorICMS: double; baseICMSST: double; valorICMSST: double; valorIPI: double; AliquotaICMS: double; AliquotaIPI: double; paramUnidade: double; descUnidade: string);
begin
...
procedure TfrmEntradasAvulsas.btAdicionarClick(Sender: TObject);
begin
  inherited;

  if verificaValidadeLancamentoItemCompra() then
  begin
    adicionaItemCompra(FormatFloat('000', dm.tbComprasTemp.RecordCount + 1), ed_cod_item.text, ed_desc_item.text, EDIquantidade.value, ed_cod_un_entrada.text, 'UN', EDIpreco.value, '000', '5102', 0, 0, 0, 0, 0, 0, 0, 0, 0, getParametroUnidade(ed_cod_un_entrada.Text), ed_desc_un_entrada.text);
...
begin
  ...
  dm.qry_entradasTOTAL_NOTA.Value := (dm.qry_entradasVALOR_PRODUTOS.Value + +dm.qry_entradasVALOR_ICMS_SUBST.Value + dm.qry_entradasVALOR_IPI.Value + dm.qry_entradasFRETE.Value + dm.qry_entradasSEGURO.Value + dm.qry_entradasOUTRAS_DESPESAS.Value) - dm.qry_entradasDESCONTO.value;
...
begin
  ...
  begin
    ...
    LNumeroNotaFiscal := dm.qry_entradasNUMERO_NOTA.Value;
...
begin
  ...
  begin
    LCodFornecedor := ed_cod_fornecedor.Text;
    LValorCompra := dm.qry_entradasTOTAL_NOTA.Value;
//   numeroNotaFiscal := edNumeroNotaFiscal.Text;
    LNomeFornecedor := ed_razao_social_fornecedor.Text;
    LValorCompra := dm.qry_entradasTOTAL_NOTA.Value;
    LDataentrada := dm.qry_entradasDATA_ENTRADA.Value;
...
begin
  ...
  try
    try
      frmContasApagarEntradas.SetCodigoFornecedor(LCodFornecedor);
      frmContasApagarEntradas.setEntradasAVulsas(true);
      frmContasApagarEntradas.edNumeroEntrada.Text := LCodigoEntrada;
      frmContasApagarEntradas.edTotalConta.Value := LValorCompra;
      frmContasApagarEntradas.edNumeroNota.Text := '00000000';
      frmContasApagarEntradas.edDataRd.Date := LDataentrada;
      frmContasApagarEntradas.edDataDoc.Date := LDataentrada;
      frmContasApagarEntradas.edDataVencimento.Date := now;

      frmContasApagarEntradas.edValorAjustado.value := LValorCompra;
...
begin
  ...
  begin
    ...
    adicionaItemCompra(q.fieldbyname('ordem').value, q.fieldbyname('cod_item').value, q.fieldbyname('descricao').value, q.fieldbyname('quantidade').value, q.fieldbyname('cod_unidade').value, q.fieldbyname('sigla_unid').value, q.fieldbyname('valor_unitario').value, q.fieldbyname('CST').value, q.fieldbyname('cfop').value, q.fieldbyname('desconto').value, q.fieldbyname('outras_despesas').value, q.fieldbyname('base_icms').value, q.fieldbyname('valor_icms').value, q.fieldbyname('base_icms_st').value, q.fieldbyname('valor_icms_st').value, q.fieldbyname('valor_ipi').value, q.fieldbyname('aliquota_icms').value, q.fieldbyname('aliquota_ipi').value, getParametroUnidade(q.fieldbyname('cod_unidade').value), q.fieldbyname('descricao_unidade').value);
    q.Next;
  end;
end;
...
procedure TfrmEntradasAvulsas.edit1KeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  inherited;
  if Key = vk_f11 then
  begin
...
procedure TfrmEntradasAvulsas.cxGrid1DBTableView1KeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  inherited;
...
begin
  ...
  if Sender is TCurrencyEdit and ((Sender as TCurrencyEdit).Name = 'EDIquantidade') then
  begin
    if EDIquantidade.Value < 0 then
    begin
  ...
  if Sender is TCurrencyEdit and ((Sender as TCurrencyEdit).Name = 'edTotalItem') then
  begin
    EDIpreco.value := (edTotalItem.value / ediQUantidade.value);
  end;

  if Sender is TCurrencyEdit and ((Sender as TCurrencyEdit).Name = 'EDIpreco') then
  begin
    edTotalItem.value := ediQUantidade.value * EDIPreco.value;
  end;
...
procedure TfrmEntradasAvulsas.DBGrid1KeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  inherited;
...
procedure TfrmEntradasAvulsas.ExcluirItem();
begin

  if (ShowRealDialog(Self, tmConfirmacao, 'Confirme', 'Confirma a exclusão do ítem ? ', 16) = teSim) then
  begin

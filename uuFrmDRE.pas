...
uses
  ...
  sSkinProvider, sSkinManager, sLabel;
...
type
  TfrmDREVersaoNova = class(Tfrm_modelo_vazio)
    ...
    lbDetalhamentoGrupo: TsLabel;
    lbTotalGrupoDetalhamento: TsLabel;
    lbPercFaturamentoDetalhamento: TsLabel;
    lbPercDespesaDetalhamento: TsLabel;
    sLabel1: TsLabel;
  ...
  private
    ...
    procedure DetalharContasAPagarDaEntrada(ACodEntrada: string);
...
begin
  ...
  CopiarPopupMenuItens(popMenuExportacaoGrid, popCompras, false);
  CopiarPopupMenuItens(popMenuExportacaoGrid, popProdutos, false);
...
begin
  ...
  try
    try
      ...
      LAnoSelecionado := StrToInt(cbAno.EditValue);
...
begin
  ...
  try
    ...
    LResultadoAnoAnoAnterior := dm.qryDRE.FieldByName('fat_ano_ant').value - dm.qryDRE.FieldByName('total_desp_ano_anterior').value;
...
begin
  ...
  dm.qryContasAPagarEntradasDRE.close;
...
begin
  pnDetalhamentoDaEntrada.Visible := false;
  dm.qryProdutosEntradaDRE.Active := false;
end;
...
begin
  inherited;
  try
    if pnDetalhamentoDaEntrada.Visible = true then
    begin
      LCodigoConta := dm.qryDRE.FieldByName('cod_conta').Value;
      LCodigoEntrada := dm.qryEntradasGrupoDRE.FieldByName('cod_entrada').Value;
      DetalharProdutosPorEntrada(LCodigoEntrada, LCodigoConta);

      DetalharContasAPagarDaEntrada(LCodigoEntrada);
...
begin
  pnDetalhamentoDaEntrada.Visible := true;
  ExibiRProdutosDaEntrada;
end;
...
procedure TfrmDREVersaoNova.DetalharContasAPagarDaEntrada(ACodEntrada: string);
var
  LSQL: Tstringlist;
  LPeriodoMesAtual: TEstruturaPeriodo;
  LMesSelecionado, LAnoSelecionado: integer;
begin
  ...
  try
    try
      ...
      LSQL.add('     left join plano_contas_financeiro pc on (pc.cod_conta = cp.cod_conta)');
      LSQL.add('     inner join parametros p on (p.cod_empresa = cp.cod_emp_grupo)');
      LSQL.add('  where cp.codigo_entrada =  ' + Quotedstr(ACodEntrada));

      LSQL.add(') ');
      LSQL.add(' where 1=1 ');

      dm.qryContasAPagarEntradasDRE.Close;
      dm.qryContasAPagarEntradasDRE.sql.clear;
      dm.qryContasAPagarEntradasDRE.sql.add(LSQL.Text);
      dm.adicionaLog(LSQL.Text);
      dm.qryContasAPagarEntradasDRE.Active := true;

    except
      on E: Exception do
      begin
        ShowRealDialog(frmDREVersaoNova, tmErro, 'Ocorreu  um erro', 'Erro ao buscar contas a pagar da entrada' + sLineBreak + sLineBreak + E.Message, 16);
      end;

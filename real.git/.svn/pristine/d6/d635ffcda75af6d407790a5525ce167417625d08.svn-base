...
type
  TfrmCsModeloNotaFiscal = class(Tfrm_modelo_pesquisas)
    Label2: TLabel;
...
procedure TfrmCsModeloNotaFiscal.FormActivate(Sender: TObject);
begin
  inherited;
  dm.qry_consultas.Active:=false;
  dm.qry_consultas.sql.Clear;
  dm.qry_consultas.sql.add('select * from modelos_nf order by codigo');
  dm.qry_consultas.Active:=true;
end;

procedure TfrmCsModeloNotaFiscal.DBGrid1DrawColumnCell(Sender: TObject;

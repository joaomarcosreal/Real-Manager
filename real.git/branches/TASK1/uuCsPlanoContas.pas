...
uses
  ...
  Dialogs, uu_modelo_Vazio, ImgList, StdCtrls, Buttons, ComCtrls, ExtCtrls, ibquery,
  Grids, DBGrids;
...
begin
 ...
 QFil.SQL.Add('select * from r_plano_contas where cod_conta_pai='+(pai)+' order by  cod_conta_raiz,cod_conta_pai,desdobramento');
 QFil.Active :=true;
 ...
 begin
  ...
  QPai.SQL.Clear;
  QPai.SQL.Add('SELECT COUNT(*) AS QUANT FROM r_plano_contas where cod_conta_pai ='+(QFil.FieldByName('cod_conta').value));
  QPai.Open;
...
begin
  ...
  dm.qry_consultas.SQL.add('select cod_formatado,descricao from r_plano_contas');
...
begin
 ...
 Q.SQL.clear;
 q.SQL.add('select cod_conta,descricao from r_plano_contas where cod_formatado='+Quotedstr(str));
 Q.Active:=true;
...
begin
  ...
  dm.qry_consultas.Active :=false;
  dm.qry_consultas.SQL.clear;
  dm.qry_consultas.SQL.add('select cod_formatado,descricao from r_plano_contas where descricao    like '+Quotedstr('%'+edDescricao.Text+'%')+' ');
  dm.qry_consultas.Active := true;

end;

procedure Tfrm_cs_contas.DBGrid1DrawColumnCell(Sender: TObject;

...
uses
  ...
  ExtCtrls, DB, IBQuery, RxCurrEdit, acbrDevice, AdvTouchKeyboard, RxToolEdit;
...
begin
  ...
  qryAuxiliarAbertura.SQL.add('insert into caixas (codigo,suprimento_inicial,data_abertura,data_sistema,hora_abertura,cod_usuario,nome_usuario,status,numero_caixa,cobra_servico,turno) values');
 ...
 qryAuxiliarAbertura.sql.add(''+IntToStr(cobra_servico) +',');
  ...
  qryAuxiliarAbertura.Active:=false;
  qryAuxiliarAbertura.sql.Clear;
  qryAuxiliarAbertura.SQL.Add('insert into suprimentos (cod_forma,valor,data,cod_usuario,cod_caixa) values (');
  qryAuxiliarAbertura.sql.add(''+QuotedStr('001')+',');
  qryAuxiliarAbertura.sql.add(''+dm.TrocaVirgPPto(trim(FloatToStr(edValorFundo.value))) +',');
  qryAuxiliarAbertura.sql.add(''+QuotedStr(FormatDateTime('DD.MM.YYY',data_do_sistema))+',');
  qryAuxiliarAbertura.sql.add(''+QuotedStr(codigo_usuario)+',');
  qryAuxiliarAbertura.sql.add(''+QUotedstr( ultimoCaixaAberto)+')');
  qryAuxiliarAbertura.ExecSQL;
  ...
  dm.exibe_painel_erro('Abertura de Caixa EFETUADA!','');
  close;
end;
...
begin
  ...
  if (data_do_sistema <> Date) then
   begin
    dm.exibe_painel_erro('Data do sistema difere da data do computador. Verifique se'
      +' o encerramento do dia já foi efetuado.!','Ok');
      exit;
   end;
            ...
            if estado <>  estNaoInicializada then
             begin
              ...
              if estado = estRequerZ then
               begin
                 dm.exibe_painel_erro('O ECF emitirá agora a redução Z, aguarde enquanto até que o ECF termine...','Tecle enter para continuar');
                ...
                begin
                 try
                   ...
                   dm.exibe_painel_erro('Erro Corrigido, tente novamente...','...');
                   exit;
                 except
                   begin
...
//        frm_principal.ECF.AberturaDoDia(codigoSuprimento,edValorFundo.Value,'Dinheiro',nome_usuario );
        frm_principal.ECF.Suprimento(edValorFundo.Value,'','SUPRIMENTO','DINHEIRO');
      end

...
uses
  ...
  Dialogs, uu_modelo, StdCtrls, Buttons, ExtCtrls, ibx.ibquery,
  Data.DB, RxCurrEdit,

  FireDAC.Comp.Client, FireDAC.Stan.Intf, FireDAC.Stan.Option,
  FireDAC.Stan.Param, FireDAC.Stan.Error, FireDAC.DatS,
  FireDAC.Comp.DataSet, RxToolEdit, FireDAC.Phys.Intf, FireDAC.DApt.Intf,
  Vcl.Mask;
...
type
  ...
  private
    procedure CalcularFichaTecnica(ACodSubProduto: string; AQuantidade: double);
...
uses
  uu_data_module, uu_modelo_vazio, uu_frm_principal;
...
procedure TfrmTransformacoesSubmateria.CalcularFichaTecnica(ACodSubProduto: string; AQuantidade: double);
var
  LSQL: TStringList;
  LTbAux: TFDMemTable;
  LRendimento: double;
  LProporcaoReceita: double;
begin
  ...
  try
    try
      LSQL.Clear;
      LSQL.Add('select rendimento from produtos p where p.cod_produto=' + Quotedstr(ACodSubProduto));
      dm.getMemTable(LTbAux, LSQL.Text);
      LRendimento := LTbAux.Fields[0].Value;
      if (LRendimento <= 0) then
      begin
        dm.exibe_painel_erro('Rendimento da ficha técnica do produto é menor ou igual a zero. Ajuste o rendimento e tente novamente!','Ok');
        raise Exception.Create('Rendimento da Ficha técnica inválido!');
      end;

      LProporcaoReceita := ((AQuantidade * 100) / LRendimento);


    except
      on E: Exception do
      begin
        dm.exibe_painel_erro('Erro ao gerar ficha de produção ', 'Ok');
        raise Exception.Create('Erro ao gerar ficha de produção');
      end;
    end;
  finally
    FreeAndNil(LSQL);

...
uses
  ...
  Dialogs, StdCtrls, ExtCtrls,   Buttons, Grids,  System.Types, System.StrUtils,
  DBGrids, ComCtrls, Mask, ImgList, CheckLst,uu_modelo_vazio, Data.DB,
  Vcl.ExtDlgs, Vcl.Menus, System.ImageList,
   Ibquery, DBCtrls, RxToolEdit, RxCurrEdit, uuDMCadProdutos, cxGraphics,
...
type
  Tfrm_produtos = class(Tfrm_modelo_vazio)
    ...
    GroupBox7: TGroupBox;
    ckbCodSistema: TRadioButton;
    ckbCodCardapio: TRadioButton;
    Label1: TLabel;
    ...
    Label54: TLabel;
    BitBtn6: TBitBtn;
    cbUnidadeFracao: TcxLookupComboBox;
    ...
    Label41: TLabel;
    Label42: TLabel;
    edCodCardapioClone: TEdit;
    edDescricaoClone: TEdit;
    edValorClone: TCurrencyEdit;
    edDescricaoReduzidaClone: TEdit;
    BitBtn2: TBitBtn;
    ...
    procedure  retornaGuarnicoes();

    procedure edCodBarrasProdutoKeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
    ...
    procedure gravar_acompanhamentos;
    ...
    procedure edDescTipoItemSpedKeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
    procedure setCodItemCadastrado(codigo : string);
    procedure edDescricaoCloneKeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
...
uses uu_data_module, uu_frm_principal, uu_cs_produtos,

  uu_cs_c_custo, uu_cs_unidades, uu_cad_itens_estoque;

{$R *.dfm}


function Tfrm_produtos.controla_botoes(botao : TBitBtn) : boolean;
begin
...
begin
  ...
  dm.tb_acompanhamentos_temp.Active:=false;
  dm.tb_acompanhamentos_temp.EmptyTable;
  dm.tb_acompanhamentos_temp.Active:=true;
...
procedure Tfrm_produtos.edCodBarrasProdutoKeyDown(Sender: TObject;
...
procedure Tfrm_produtos.edCodProdutoExit(Sender: TObject);
var
 retorno : string;
 filtro : string;
begin
   ...
   if  sender is Tedit and ((sender as Tedit).Name='edCodProduto') then
    begin
     if trim(edCodProduto.Text) = '' then exit;
      ...
      if ckbCodCardapio.Checked then
       begin
         if (verificaExistenciaProduto(trim(edCodProduto.Text),2) = true) then
          begin
            edCodProduto.Text := getCodProduto(edCodProduto.Text);
          end
         else
           begin
...
begin
  ...
  dm.composicoes_tempdes_unidade.Text:= cbUnidadeFichaTecnica.EditText;
...
procedure Tfrm_produtos.btPesqProdutosClick(Sender: TObject);
begin
  inherited;
...
function Tfrm_produtos.gravar_informacoes : boolean;
var
 ...
 lucro_maximo, impostos, outrasTaxas,maoDeObra, lucro_minimo, obs_lancamento,tempo_preparo,cod_miniPrinter,vendeFracionado, pizza,tamanho_pizza,produtoBalanca,precoDigitado,informarSabor,controlaEstoque,comissionado,incideServico,couvert,valorComissao : string;
 descricaoReduzida : string; parametroContabil : string; custoDigitado : string; calculaQtde : string; impBalcao : string; impMesas : string; impDeliver : string;
...
begin
  ...
  descricaoReduzida:= Quotedstr(trim(edDescricaoReduzida.Text));
...
//  caminho_foto:=QuotedStr('c:\restaurante\imagens\semfoto.jpg');
  ...
  case rdTamanhoPizza.ItemIndex of
    0 : tamanho_pizza := Quotedstr('B');
    1 : tamanho_pizza := Quotedstr('P');
    2 : tamanho_pizza := Quotedstr('M');
    3 : tamanho_pizza := Quotedstr('G');
    4 : tamanho_pizza := Quotedstr('F');
  end;
 ...
 unidadeFracao := Quotedstr(cbUnidadeFracao.EditValue);
 ...
 caminho_foto := Quotedstr('');
  ...
  1: begin
      ...
      if StrToInt(vende) = 0 then
       ...
       dm.qryauxiliar.sql.add('valor_unitario,valor_unitario_02,lucro_maximo,impostos,outras_taxas,mao_obra, cod_aliquota, REDUCAO_BASE_ICMS, caminho_foto,cod_barras,');
       ...
       dm.qryauxiliar.SQL.add('CFOP_TRANSFERENCIAS,CST_TRANSFERENCIAS,CFOP_DEVOLUCAO,CST_DEVOLUCAO,CST_PIS_COFINS,ALIQ_PIS,ALIQ_COFINS,cod_grupo_touch');
  ...
  3: begin
       ...
       begin
          ...
          dm.qryauxiliar2.SQL.Add('delete from guarnicoes_produto where cod_produto='+quotedstr(edCodProduto.Text));
...
//   1: showmessage('Produto Cadastrado com sucesso!');
   2: showmessage('Produto alterado com sucesso!');
   3: showmessage('Produto excluído com sucesso!');
  end
...
procedure Tfrm_produtos.gravar_itens_produtos;
var
...
procedure Tfrm_produtos.FormActivate(Sender: TObject);
begin
  bt_cancelar.Click;
...
begin
   ...
   try
     cbUnidadeFracao.EditValue       := dm.qryauxiliar.fieldbyname('cod_unidade_fracao').value;
   except
     cbUnidadeFracao.EditValue := '10';
   end;
   ...
   try
     edEstoqueAtual.Value     := dm.qryauxiliar.fieldbyname('estoque').value;
   except
     edEstoqueAtual.Value := 1;
   end;
...
procedure Tfrm_produtos.retornaGuarnicoes();
begin
  dm.qryauxiliar2.Active:=false;
  dm.qryauxiliar2.SQL.Clear;
  dm.qryauxiliar2.sql.add('  select gp.cod_guarnicao,g.descricao  from guarnicoes_produto gp ');
  dm.qryauxiliar2.sql.add('    inner join guarnicoes g on (g.codigo = gp.cod_guarnicao) ');
  dm.qryauxiliar2.sql.add('     where gp.cod_produto='+Quotedstr(edCodProduto.Text));
  dm.qryauxiliar2.sql.add('    order by gp.cod_guarnicao');
  dm.qryauxiliar2.Active := true;
  dm.qryauxiliar2.First;

  dm.tbGuarnicoesTemp.EmptyTable;
  dm.tbGuarnicoesTemp.Active := true;

  while not dm.qryauxiliar2.Eof do
   begin
     dm.tbGuarnicoesTemp.Append;
     dm.tbGuarnicoesTempcodigo.Value := dm.qryauxiliar2.fieldbyname('cod_guarnicao').value;
     dm.tbGuarnicoesTempdescricao.Value := dm.qryauxiliar2.fieldbyname('descricao').value;
     dm.tbGuarnicoesTemp.Post;
...
procedure Tfrm_produtos.bt_novoClick(Sender: TObject);
begin
...
procedure Tfrm_produtos.bt_alterarClick(Sender: TObject);
begin
 oquefazer:=2;
...
procedure Tfrm_produtos.bt_excluirClick(Sender: TObject);
begin
...
procedure Tfrm_produtos.bt_gravarClick(Sender: TObject);
begin
...
procedure Tfrm_produtos.bt_cancelarClick(Sender: TObject);
begin
...
procedure Tfrm_produtos.gravar_acompanhamentos;
begin
  dm.qryauxiliar.Active:=false;
  dm.qryauxiliar.sql.clear;
  dm.qryauxiliar.SQL.add('delete from acompanhamentos where cod_produto='+Quotedstr(edCodProduto.Text));
  try
    begin
     dm.qryauxiliar.ExecSQL;
    end;
    ...
    begin
      dm.cria_log_de_erros(dm.qryauxiliar.sql,'Cadastro de Produtos ','Erro excluir acompanhamentos para atualização ',codigo_usuario);
      exit;
    end;
  end;

  dm.tb_acompanhamentos_temp.first;
  while not dm.tb_acompanhamentos_temp.Eof do
  begin
    dm.qryauxiliar.Active:=false;
    dm.qryauxiliar.sql.clear;
    dm.qryauxiliar.sql.add('insert into acompanhamentos(cod_produto,cod_produto_acom,qtde) values');
    dm.qryauxiliar.sql.add('('+Quotedstr(edCodProduto
    .text)+','+QuotedStr(dm.tb_acompanhamentos_tempcod_acompanhamento.Text)+','+IntToStr(dm.tb_acompanhamentos_tempqtde.Value)+')');
    try
      begin
        dm.qryauxiliar.ExecSQL;
      end;
      ...
      begin
        ...
        dm.cria_log_de_erros(dm.qryauxiliar.sql,'Cadastro de Produtos ','Erro gravar acompanhamentos ',codigo_usuario);
        exit;
      end;
    end;
    dm.tb_acompanhamentos_temp.Next;
  end;
end;
...
procedure Tfrm_produtos.BitBtn2Click(Sender: TObject);
var
 sqlClone : string;
 ...
 arquivoImportacao :  TStrings;
 linhaAtualArquivo : string;
begin
  inherited;
  ...
  arquivoImportacao.LoadFromFile('c:\restaurante\saporra.csv');

  for linhaAtualArquivo in  arquivoImportacao do
   begin
     CodigoProduto :=dm.geraCodigo('G_PRODUTOS',5);
     campos := SplitString(linhaAtualArquivo,';');
     edDescricaoClone.Text   := campos[1];
//     edCodCardapioClone.Text := campos[2];
     edValorClone.Value        := StrToFloat( dm.TrocaPontoPVirgula(campos[2]));
     precoClone :=Quotedstr(dm.TrocaVirgPPto(Floattostr(edValorClone.Value) ));

     if Length (edCodCardapioClone.Text) <> 13 then
       edCodCardapioClone.Text := '';

     try

        if trim(edCodCardapioClone.Text) = ''  then
          codBarrasClone := codigoProduto
        else
          codBarrasClone := FormatFloat('00000', StrToFloat(edCodCardapioClone.Text));


        sqlClone := 'INSERT INTO PRODUTOS (COD_PRODUTO,COD_BARRAS, DESCRICAO,DESCRICAO_REDUZIDA, VALOR_UNITARIO, DATA_CADASTRO, ULTIMA_VENDA,'
                    +'       NUMERO_VENDAS, COD_ALIQUOTA, CAMINHO_FOTO,  ACOMPANHAMENTO,'
                    +'       PERMITE_ACOMPANHAMENTO, ACOMPANHAMENTOS_FIXOS, NUMERO_ACOMPANHAMENTOS,'
                    +'       LUCRO_MAXIMO, LUCRO_MINIMO, PRODUTO_FINAL, ITEM_ASSOCIADO, COD_GRUPO,'
                    +'       CARDAPIO, TEMPO, OBS_LANCAMENTO, COD_MINIPRINTER, PARAMETRO_CONTAGEM,'
                    +'       BALANCA, FRACIONADO, PRECO_DIGITADO, INFORMAR_SABOR, COMISSIONADO,'
                    +'       VALOR_COMISSAO, INCIDE_SERVICO, COUVERT, CUSTODIGITADO,'
                    +'       CALCULA_QTDE, PIZZA, TAM_PIZZA, IMP_DELIVERY, IMP_MESAS, IMP_BALCAO, COMPRA,'
                    +'       VENDE, COD_TIPO_ITEM, COD_GENERO, NCM, IPPT, IAT, DATA_ULTIMA_REPOSICAO,'
                    +'       VALOR_CUSTO, QUANTIDADE_MAXIMA, QUANTIDADE_MINIMA, ESTOQUE_LIQUIDO, UNIDADE_ENTRADA,'
                    +'       COD_UNIDADE_FRACAO, CFOP, CST, CFOP_SAIDA, CST_SAIDA, UN_ECF, SUBMATERIA,'
                    +'       CFOP_TRANSFERENCIAS, CST_TRANSFERENCIAS, CFOP_DEVOLUCAO, CST_DEVOLUCAO, '
                    +'       CST_PIS_COFINS, ALIQ_PIS, ALIQ_COFINS, ESTOQUE_VISIVEL, QTD_GUARNICOES,'
                    +'       GUARNICOES_FIXAS, COD_GRUPO_TOUCH, REQUER_OBSERVACAO, imprimir_ficha, '
                    +'       COD_GRUPO_APURACAO, RENDIMENTO,perc_perda, BALANCA_LEITURA, VALOR_UNITARIO_02,'
                    +'       CEST, REDUCAO_BASE_ICMS)'
                    +'      select'
                    +'       '+Quotedstr(codigoProduto)+','
                    +'       '+Quotedstr(codBarrasClone)+','
                              +QuotedStr(edDescricaoClone.text)+','
                              +QuotedStr(edDescricaoReduzidaClone.text)
                              +','
                              +precoClone+', DATA_CADASTRO, ULTIMA_VENDA,'
                    +'       NUMERO_VENDAS, COD_ALIQUOTA, CAMINHO_FOTO , ACOMPANHAMENTO,'
                    +'       PERMITE_ACOMPANHAMENTO, ACOMPANHAMENTOS_FIXOS, NUMERO_ACOMPANHAMENTOS,'
                    +'       LUCRO_MAXIMO, LUCRO_MINIMO, PRODUTO_FINAL, ITEM_ASSOCIADO, COD_GRUPO,'
                    +'       CARDAPIO, TEMPO, OBS_LANCAMENTO, COD_MINIPRINTER, PARAMETRO_CONTAGEM,'
                    ...
                    +'       GUARNICOES_FIXAS, COD_GRUPO_TOUCH, REQUER_OBSERVACAO,imprimir_ficha, '
                    +'       COD_GRUPO_APURACAO, RENDIMENTO, perc_perda, BALANCA_LEITURA, VALOR_UNITARIO_02,'
                    +'       CEST, REDUCAO_BASE_ICMS from produtos p ';

        dm.qryauxiliar.Active := false;
        dm.qryauxiliar.SQL.Clear;
        dm.qryauxiliar.SQL.Add(sqlClone);
        dm.qryauxiliar.SQL.Add('where p.cod_produto='+Quotedstr(edCodProduto.Text));
        ...
        edDescricaoReduzidaClone.Clear;
        edCodCardapioClone.Clear;
        edValorClone.Value :=0;
       ...
       on E : Exception  do
        begin
          ...
          dm.adicionaLog('Erro na linha: '+linhaAtualArquivo);
        end;
     end;
   end;
...
procedure Tfrm_produtos.edDescTipoItemSpedKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  inherited;
 if key = vk_f11 then
...
procedure Tfrm_produtos.Edit1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  inherited;
...
procedure Tfrm_produtos.edDescricaoCloneKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if ( key = vk_f2 ) then
   begin
...
procedure Tfrm_produtos.BitBtn4Click(Sender: TObject);
var
   i : integer;
   codigoGrupo : string;
   codSabor : string;
...
begin
  ...
  frm_principal.memoSabores.Lines.Clear;
  frm_principal.memoSabores.Lines := memoSabores.Lines;



  for i := 0 to frm_principal.memoSabores.Lines.Count-1 do
     begin
       if (trim(frm_principal.memoSabores.Lines[i]) <> '') then
        begin
           ...
           if (precoBT > 0 ) then
            begin
             ...
             cbGrupoTouch.EditValue := '01';
             ckbVende.Checked := true;
             ed_descricao_produto.Text :='PIZZA '+  frm_principal.memoSabores.Lines[i];
          ...
          DM.qryauxiliar.SQL.Add(''+QUOTEDSTR(UpperCase ( frm_principal.memoSabores.Lines[i]))+',');
...
procedure Tfrm_produtos.BitBtn6Click(Sender: TObject);
var
 sqlClone : string;
 codigoProduto : string;
 precoClone : string;
...
begin
  ...
  if trim(edDescricaoClone.Text) = ''  then
   begin
     ShowMessage('Descrição do clone inválida!');
     exit;
   end;


  if trim(edDescricaoReduzida.Text) = ''  then
   begin
     ShowMessage('Descrição reduzida do clone inválida!');
     exit;
   end;


  if  edValorClone.Value <= 0  then
   begin
     ShowMessage('Preço do clone inválido!');
     exit;
   end;
  ...
  CodigoProduto :=dm.geraCodigo('G_PRODUTOS',5);
          ...
          sqlClone := 'INSERT INTO PRODUTOS (COD_PRODUTO,COD_BARRAS, DESCRICAO,DESCRICAO_REDUZIDA, VALOR_UNITARIO, DATA_CADASTRO, ULTIMA_VENDA,'
                      ...
                      +'       VENDE, COD_TIPO_ITEM, COD_GENERO, NCM, IPPT, IAT, DATA_ULTIMA_REPOSICAO,'
                      +'       VALOR_CUSTO, QUANTIDADE_MAXIMA, QUANTIDADE_MINIMA, ESTOQUE_LIQUIDO, UNIDADE_ENTRADA,'
                      +'       COD_UNIDADE_FRACAO,CEST,CFOP, CST, CFOP_SAIDA, CST_SAIDA, UN_ECF, SUBMATERIA,'
                      +'       CFOP_TRANSFERENCIAS, CST_TRANSFERENCIAS, CFOP_DEVOLUCAO, CST_DEVOLUCAO, '
                      +'       CST_PIS_COFINS, ALIQ_PIS, ALIQ_COFINS, ESTOQUE_VISIVEL, QTD_GUARNICOES,'
                      +'       GUARNICOES_FIXAS, COD_GRUPO_TOUCH, REQUER_OBSERVACAO, imprimir_ficha, '
                      +'       COD_GRUPO_APURACAO, RENDIMENTO,perc_perda, BALANCA_LEITURA, VALOR_UNITARIO_02,'
                      ...
                      +'       GUARNICOES_FIXAS, COD_GRUPO_TOUCH, REQUER_OBSERVACAO,imprimir_ficha, '
                      +'       COD_GRUPO_APURACAO, RENDIMENTO, perc_perda, BALANCA_LEITURA, VALOR_UNITARIO_02,'
                      +'       REDUCAO_BASE_ICMS from produtos p ';

...
function udf_TruncECF(var X: double; var Decimal: SmallInt): double; cdecl; export;
var
  Factor: integer;
  fracao : integer;
begin
...
procedure adicionaLog(local: string; observacao: string);
var arq: TextFile;
linha: String;
...
begin
  ...
  nomeDoArquivo := local+'logs\'+FormatDateTime('YYY',Date)+'_'+FormatDateTime('MM',date)+'_'+FormatDateTime('DD',Date)+'.log';
...
//  adicionaLog('c:\','Número de entrada: '+FloatToStr(AValue));
//  adicionaLog('c:\','Dígitos de entrada: '+Inttostr(Digits));

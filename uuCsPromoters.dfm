inherited frmCsPromoters: TfrmCsPromoters
  Left = 391
  Top = 92
  Caption = 'Consulta de Promoters'
  PixelsPerInch = 96
  TextHeight = 13
  inherited Panel3: TPanel
    Height = 58
    ExplicitHeight = 58
    inherited Label1: TLabel
      Top = 4
      Width = 233
      Height = 20
      Caption = 'Digite as iniciais do promoter'
      Font.Height = -16
      ExplicitTop = 4
      ExplicitWidth = 233
      ExplicitHeight = 20
    end
    inherited ed_parametro: TEdit
      Top = 25
      Width = 422
      Height = 28
      Font.Height = -16
      ParentFont = False
      ExplicitTop = 25
      ExplicitWidth = 422
      ExplicitHeight = 28
    end
    inherited RadioGroup1: TRadioGroup
      Top = -58
      ExplicitTop = -58
    end
  end
  inherited Panel1: TPanel
    Top = 58
    Height = 127
    ExplicitTop = 58
    ExplicitHeight = 127
    inherited DBGrid1: TDBGrid
      Width = 452
      Height = 309
      Font.Height = -16
      OnDrawColumnCell = DBGrid1DrawColumnCell
      Columns = <
        item
          Expanded = False
          FieldName = 'COD_PROMOTER'
          Title.Caption = 'C'#243'digo'
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -16
          Title.Font.Name = 'MS Sans Serif'
          Title.Font.Style = [fsBold]
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'NOME'
          Title.Caption = 'Nome do Promoter'
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -16
          Title.Font.Name = 'MS Sans Serif'
          Title.Font.Style = [fsBold]
          Width = 350
          Visible = True
        end>
    end
  end
  inherited Panel2: TPanel
    Top = 185
    Height = 57
    ExplicitTop = 185
    ExplicitHeight = 57
    inherited bt_selecionar: TBitBtn
      Top = 2
      Width = 228
      Height = 53
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      ExplicitTop = 2
      ExplicitWidth = 228
      ExplicitHeight = 53
    end
    inherited bt_cancelar: TBitBtn
      Left = 230
      Top = 3
      Width = 228
      Height = 52
      Glyph.Data = {
        
        8786918F8EB4B4B4EEEEEEFCFCFCFFFFFFFFFFFFFFFFFFFFFFFF}
      ExplicitLeft = 230
      ExplicitTop = 3
      ExplicitWidth = 228
      ExplicitHeight = 52
    end
  end
end
